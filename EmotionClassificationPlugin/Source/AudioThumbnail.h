 ...
 * @brief Audio Thumbnail from the JUCE tutorial with added sectioning and labels for each section
 * @version 0.1
 * @date 2023-05-08
 *
 *
...
{
public:
    AudioThumbnailComponent(float sectionLength = 0, std::string nofiletext = "No File Loaded")
       : thumbnailCache (5),
    ...
    {
        auto backcolor = getLookAndFeel().findColour (juce::ResizableWindow::backgroundColourId);
        ...
        Path p;
        p.addRectangle (rectbox.getX(), rectbox.getY(), rectbox.getWidth(), rectbox.getHeight());
    ...
    void setLabels(const std::vector<std::string>& labels, const std::vector<juce::Colour>& colors = {}) {
...
private:
    ...
    void paintIfNoFileLoaded (juce::Graphics& g, const juce::Rectangle<int>& thumbnailBounds)
    {
        ...
        g.setColour (juce::Colours::white);
        g.drawFittedText (nofileText, thumbnailBounds, juce::Justification::centred, 1);
    }

    void drawVline (juce::Graphics& g, float lineX, float lineY, float lineH, float lineThickness = 1.f, juce::Colour color = juce::Colours::darkred) {
        g.setColour (color);
        Path p;
        float lineW = 1.0f;
    ...
    void paintIfFileLoaded (juce::Graphics& g, const juce::Rectangle<int>& thumbnailBounds)
    {
        ...
        if (sectionsLength > 0) {
            ...
            float magnification = thumbNailWidth / thumbNailLength; // Ratio between the length of the representation in pixels and in seconds

            if (numSections > 0)
            {
                int linesToDraw = numSections + 1;
                for (int i = 0; i < linesToDraw; i++) {
                    float lineX = thumbNailX + (i * sectionsLength * magnification);
                    drawVline (g, lineX, thumbNailY, thumbNailHeight, 1.f, juce::Colours::cyan);
                }
                ...
                if (sectionLabels.size() > 0) {
                    ...
                    for (int i = 0; i < sectionLabels.size(); i++) {
                        ...
                        g.drawFittedText (sectionLabels[i], thumbNailX + (i * sectionsLength * magnification), thumbNailY+thumbNailHeight, sectionsLength*magnification, 20, juce::Justification::centred, 2);
                    }
                }
            }
        }
